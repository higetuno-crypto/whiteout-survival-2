...
// 柵が完成済みなら、解錠済み隣接エリア向きのゲートアーチも復元(演出なし)。
syncGateArches(false);

// farm/ranchPenのsite固有ストックを復元(サイトが存在する=対応エリア解錠済みのときだけ)。
buildMgr.sites.get('farm')?.restoreStock(save.farmStock);
...
function collectSave() {
  return {
    ...
    farmStock: buildMgr.sites.get('farm')?.stock ?? 0,   // FB1: 農場の内部ストック(旧fishHutStock)
    ranchFed: buildMgr.sites.get('ranchpen')?.fed ?? 0,
    ranchPending: buildMgr.sites.get('ranchpen')?.pendingGoods ?? 0,
    depotStored: { ...(buildMgr.sites.get('depot')?.stored ?? { log: 0, rawFish: 0, cookedFish: 0, wheat: 0 }) },
  };
}
...
// FB1 農場(farm完成後、半径2m内で0.1秒ごとに小麦ストックを容量分だけ収穫。旧fishHut収穫の後継)。
const farmCollect = new ProximityAction({ radius: 2.0, startDelay: 0, interval: 0.1, requireStill: false });
...
//      (cook/merchant)が既存設備(焚き火・売店)を運転して行う(npc.js。バッチC/Dで配線)。
...
function syncGateArches(animated) {
  ...
  for (const other of AREAS) {
    if (other.id === 'camp' || !unlockedAreas.includes(other.id)) continue;
    if (!areAreasAdjacent(CAMP_AREA, other)) continue;
    const dx = Math.sign(other.cx - CAMP_AREA.cx), dz = Math.sign(other.cz - CAMP_AREA.cz);
    const gx = CAMP_AREA.cx + dx * CAMP_AREA.hw, gz = CAMP_AREA.cz + dz * CAMP_AREA.hd;
    world.ensureGateArch(`camp:${other.id}`, gx, gz, dx !== 0, animated);
  }
}
...
function step(dt) {
  ...
  // FB1 農場(farm完成後、半径2m内で0.1秒ごとに自動成長した小麦ストックを1つずつ収穫→eco.add('wheat',1)。
  // 農夫NPCが主役だが、雇う前でもプレイヤーが歩いて収穫→売店で売れる(序盤の資金源)。容量分だけ取る。
  const farmSite = buildMgr.sites.get('farm');
  if (farmSite) {
    const fd = Math.hypot(player.root.position.x - farmSite.x, player.root.position.z - farmSite.z);
    const atFarm = farmSite.completed && fd <= farmCollect.radius && farmSite.stock > 0;
    const collectTicks = farmCollect.update(atFarm, true, dt);
    for (let i = 0; i < collectTicks; i++) {
      if (farmSite.stock <= 0) break;
      const got = eco.add('wheat', 1);
      if (got <= 0) break; // 容量いっぱい
      farmSite.stock -= 1;
      farmSite.setStockVisual(farmSite.stock);
      farmSite.spawnItemFlight('wheat', farmSite.stockAnchor, new THREE.Vector3(player.root.position.x, 1.8, player.root.position.z));
    }
  }
