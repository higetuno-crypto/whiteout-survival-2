...
const FARM_STOCK_CAP = 12;   // FB1 農場の内部ストック上限(放置で貯まる小麦)
const FARM_GROW_SEC = 3;     // 農場が小麦を1つ自動生産する間隔(秒)
...
export class BuildSite {
  ...
  // n=this.stock の現在値を表示へ反映(変化時のみ再構築。表示上限5+「絵文字N」オーバーフロー表示)
  // FB1: 農場(farm)は小麦の束で積む。それ以外(旧fishHut)は生魚。
  setStockVisual(n) {
    if (n === this._stockLastShown) return;
    this._stockLastShown = n;
    const stockKind = this.f.kind === 'farm' ? 'wheat' : 'rawFish';
    const cap = 5;
    ...
    while (this._stockMeshes.length < shown) {
      ...
      const m = this._stockPool.pop() ?? createKindMesh(stockKind);
    ...
    if (this._stockSprite) {
      ...
      if (overflow) this._drawStockText(`${RESOURCES[stockKind].emoji}${n}`);
    }
  }
  ...
  restoreStock(n) {
    this.stock = Math.max(0, Math.min(FARM_STOCK_CAP, Math.trunc(n) || 0));
  ...
  update(dt) {
    ...
    if (!this.completed && this.progress >= this.f.costLogs && this.flights.length === 0) {
      this._complete();
      sfx.complete(this.x, this.z);            // 自然完成のみ(forceComplete=セーブ復元/チートでは無し)
      confetti.burst(this.x, 1.8, this.z, 20); // 完成のミニ紙吹雪(G5)
    }
    ...
    // FB1 農場: 完成後、FARM_GROW_SEC 毎に小麦ストック+1(上限FARM_STOCK_CAP。放置で貯まる=idle)。
    // 農夫(farmer NPC)がここから収穫して倉庫へ運ぶ。プレイヤー近接に関係なく自動生産。
    if (this.f.kind === 'farm' && this.completed) {
      this._stockTimer += dt;
      while (this._stockTimer >= FARM_GROW_SEC) {
        this._stockTimer -= FARM_GROW_SEC;
        if (this.stock < FARM_STOCK_CAP) this.stock++;
      }
